...
module HybridSearch
  ...
  # staleness. Used by `hybrid_search:doctor`.
  class Doctor
    ...
    def check_adapter
      if postgres?
        ...
        Check.new(name: "database_adapter", status: :error,
                  message: "HybridSearch requires PostgreSQL. Detected: #{ActiveRecord::Base.connection.adapter_name}.")
      end
    end
      ...
      if row
        ...
        Check.new(name: "pgvector_extension", status: :warning,
                  message: "pgvector extension not installed. Semantic search is unavailable until " \
                           "`CREATE EXTENSION vector;` is run (see the hybrid_search:index generator).")
      end
    end
