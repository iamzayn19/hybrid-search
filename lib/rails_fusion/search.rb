...
module RailsFusion
  ...
  class Search
    ...
    def fetch_definition!
      definition = @model_class.rails_fusion_definition
      unless definition
    ...
    def resolve_options
      config = RailsFusion.configuration
    ...
    def fetch_semantic_candidates(definition, base_scope, options)
      ...
      if provider.nil?
        message = "No embedding provider configured for #{@model_class}. " \
                  "Set `RailsFusion.configure { |c| c.embedding_provider = ... }`."
    ...
    def semantic_degraded(_definition, reason, error)
      failure_mode = RailsFusion.configuration.semantic_failure
