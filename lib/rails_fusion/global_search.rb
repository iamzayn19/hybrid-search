...
module RailsFusion
  # A single result from RailsFusion.search across multiple models.
  ...
  class GlobalSearch
    ...
    def initialize(query, models:, where: {}, limit: nil, fail_fast: false, clock: nil, **per_model_options)
      ...
      @models = models
      @where = where
      @limit = limit || RailsFusion.configuration.default_limit
      @fail_fast = fail_fast
      @clock = clock
    ...
    def fuse(per_model)
      rrf_k = RailsFusion.configuration.rrf_k
  ...
  # Result collection returned by RailsFusion.search.
