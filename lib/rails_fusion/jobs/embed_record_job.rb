  ...
  module Jobs
    ...
    class EmbedRecordJob < ActiveJob::Base
      ...
      def digest_still_matches?(record, definition, expected_digest)
        RailsFusion::Embeddings.source_digest(record, definition) == expected_digest
      end

      def generate_vector(record, definition)
        provider = RailsFusion::Embeddings.resolve_provider(definition)
        unless provider
          raise MissingEmbeddingProviderError,
                "No embedding provider configured for #{definition.model_class}."
        end

        text = RailsFusion::Embeddings.canonical_source_text(record, definition)
